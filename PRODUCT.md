...
- The creator’s open-participation/no-manual-review instruction supersedes the previous pending-first architecture. The local implementation uses automated checks and direct removal, not a review queue. Its actual boundaries are recorded in [PRIVACY_AND_MODERATION.md](PRIVACY_AND_MODERATION.md). The creator selected Cloudflare Workers + D1 for public hosting on 2026-09-10. Keep the existing React experience, use the free plan, start with an empty hosted Wall, and do not import local contributions. Any additional external processors need separate authorization.
