...
## 2026-09-10 — Prepare the requested custom hostname

The creator requested timepathy.markmathew.com, with markmathew.com registered at Namecheap, and confirmed that its existing email forwarding and website redirect are unused. Preserve that exact hostname spelling while retaining Timempathy as the project name. The planned free route keeps registration at Namecheap and moves DNS management to Cloudflare. Actual nameservers must come from the account's domain onboarding; a CNAME to workers.dev is not a substitute for a Worker Custom Domain.

The deployed Wall now accepts its existing address and the requested custom origin. Each write must still originate from the exact address receiving it, including when both addresses are approved. There is no wildcard host or cross-origin write permission. Both sites use the existing D1 database and removal keys, while optional private browser storage stays with its original origin. Keeping workers.dev available avoids forcing a move of private words.

TypeScript, deployment bundling and isolated real-D1 tests pass, including sharing at the custom hostname, reading and withdrawing at the original hostname, and rejecting unrelated hosts, HTTP and cross-origin writes. Those tests exposed the developer's .dev.vars overriding the published-origin simulation; the official harness now resolves configuration from an isolated test directory and asserts the actual origin bindings. The independent-browser sharing, age, clock, accessibility and removal journey passes. After deployment, the existing HTTPS page and public feed return 200 with no browser errors; a cross-origin write returns 403 without creating a contribution.

Preparation is deployed as Worker version 92c77e89-cb76-43e0-86a0-34f0e9a734a3. Custom-domain activation is pending the owner's Cloudflare onboarding and Namecheap nameserver change. DNS, certificate issuance and the new public address have not yet been claimed as complete. The runbook records the remaining steps; the README and GitHub homepage continue to use the verified live address.

## 2026-09-10 — Activate timepathy.markmathew.com

The creator authorized completing the domain connection through the signed-in browser. Cloudflare Free imported the existing records, which were checked against Namecheap. Root parking, the www parking CNAME and mail records were retained, with root and www set to DNS only. Namecheap had no email aliases and DNSSEC was off. Registration stays at Namecheap; its nameserver setting now points to the assigned elaine.ns.cloudflare.com and paul.ns.cloudflare.com. Public DNS and the Cloudflare dashboard confirmed activation.

The existing timempathy Worker now has the exact requested custom hostname, with DNS and HTTPS provisioned by Cloudflare. Wrangler records the custom route and assigned zone ID explicitly. The existing project-local OAuth deployed this configuration without broader authorization. The custom hostname is the primary public origin; workers.dev stays available against the same D1 database. There is no forced redirect or private-storage migration.

Release f3ce7003-cbff-4e0f-b64b-f38ef0579f28 passed deployment validation and live HTTPS checks. A labelled temporary hope was shared from the new-domain UI, read through both addresses, immediately withdrawn, and confirmed absent from both public feeds. Cross-origin writes between approved hosts still returned 403. Desktop and phone rendering were inspected with no recorded browser errors. The README, current review and deployment runbook now use the verified custom hostname. No application code, database schema, runtime dependency or paid service was added.

## 2026-09-10 — Correct the public hostname spelling

The creator corrected the address to **timempathy.markmathew.com**. The Worker Custom Domain and primary API origin now use that spelling; the earlier misspelled custom route is replaced. The original workers.dev address remains available with the same D1 database. README, deployment notes, current review, domain integration fixture and GitHub website link follow the correction. The creator also explicitly made the GitHub repository public after a tracked-history credential check.

Release b14b26ff-1445-4623-8404-c668c4779cef deployed successfully. The corrected HTTPS page and Wall API return 200; same-origin malformed submissions reach validation (400), while requests with the former origin are rejected (403). These checks create no contribution. Edge loaded the actual experience and the shared Wall at the corrected address. No application code or database schema changed.
