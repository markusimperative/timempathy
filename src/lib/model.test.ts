...
import { arcPoint, forgetReflection, readReflection, reflectionSchema, saveReflection, STORAGE_KEY, yearShare } from './model'
...
describe('the proportional lens', () => {
  it('preserves the same year across ages, without a lifespan assumption', () => {
    expect(yearShare(5)).toBe(.2)
    expect(yearShare(100)).toBe(.01)
    expect(yearShare(1)).toBe(1)
    expect(arcPoint(5, 0)).toEqual(arcPoint(85, 0))
    const end = arcPoint(5, 1)
    expect(end.x).toBeCloseTo(305.539)
    expect(end.y).toBeCloseTo(139.210)
  })
  it.each([0, -1, 32.5, NaN, Infinity, 121])('rejects invalid age %s', age => expect(() => yearShare(age)).toThrow())
...
describe('a single private reflection', () => {
  it('requires words and permits no age', () => {
    ...
    expect(reflectionSchema.parse({ text: ' A quiet lunch. ', age: null })).toEqual({ text: 'A quiet lunch.', age: null })
  ...
  it.each(['not json', '{"age":32}', '{"text":"secret","age":-1}', 'null'])('handles malformed stored data without exposing it: %s', raw => {
    const storage = memoryStorage()
