...
export function yearShare(age: number) { return 1 / ageSchema.parse(age) }
...
export const reflectionSchema = z.object({
  text: z.string().trim().min(1, 'A few words are enough.').max(240, 'Please keep your reflection within 240 characters.'),
...
export function saveReflection(storage: StoragePort, input: Reflection): boolean {
  try { storage.setItem(STORAGE_KEY, JSON.stringify(reflectionSchema.parse(input))); return true }
  catch { return false }
}
